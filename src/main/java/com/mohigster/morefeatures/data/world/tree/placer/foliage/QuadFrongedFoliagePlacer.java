...
public class QuadFrongedFoliagePlacer extends FoliagePlacer {
    public static final MapCodec<QuadFrongedFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(
            inst -> foliagePlacerParts(inst).apply(inst, QuadFrongedFoliagePlacer::new)
    );

    // How many blocks each frond arm extends outward from the crown center.
    private static final int FROND_LENGTH = 4;

    public QuadFrongedFoliagePlacer(IntProvider radius, IntProvider offset) {
        super(radius, offset);
    }
    ...
    protected FoliagePlacerType<?> type() {
        return MFFoliagePlacerType.QUAD_FRONGED_FOLIAGE_PLACER.get();
    }

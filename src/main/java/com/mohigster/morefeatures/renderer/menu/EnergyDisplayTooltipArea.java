package com.mohigster.morefeatures.renderer.menu;
...
public class EnergyDisplayTooltipArea {
    ...
    public List<Component> getTooltips() {
        return List.of(Component.literal(this.energy.getAmountAsInt() + " / " + this.energy.getCapacityAsInt()+" FE"));
    }

    public void render(GuiGraphicsExtractor guiGraphics) {
        int stored = (int)(this.height * (this.energy.getAmountAsInt() / (float)this.energy.getCapacityAsInt()));
        guiGraphics.fillGradient(xPos,yPos + (height - stored),xPos + width,
                this.yPos + height,0xffb51500, 0xff600b00);
    }
}
